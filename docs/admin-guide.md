...
| `/blocklist/stats` | Two things: the day-by-day threat totals broken down by CrowdSec scenario, with a per-day total, over the days still kept (`THREAT_EVENT_RETENTION`, a week by default); and what each machine contributed — including how much of what it sent was discarded, and for which reason. The daily totals are recomputed at most every five minutes, whatever the page traffic, and the page says when they were last computed. |
