...
- **Non-public addresses are dropped at ingest, not at read.** Private,
  ...
  `192.0.0.0/24`, `240.0.0.0/4` (broadcast included), `192.88.99.0/24`, the
  IPv6 benchmark range (`2001:2::/48`), the discard-only prefix (`100::/64`),
  Teredo (`2001::/32`) and the four IPv4-embedding prefixes — 6to4
  (`2002::/16`), NAT64 (`64:ff9b::/96` and RFC 8215's `64:ff9b:1::/48`) and
  IPv4-compatible (`::/96`) — never reach the database, so no later query, no
  export and no bug in a read path can publish one. The embedding prefixes
  matter because they carry a v4 address verbatim in their low bits:
  `2002:c0a8:0101::1` and `::192.168.1.1` both *are* `192.168.1.1`, and
  `::169.254.169.254` is the IMDS address that `IsLinkLocalUnicast` says
  nothing about. They are rejected wholesale rather than unwrapped and
  re-tested, because a v6 address whose only meaning is the v4 inside it is
  useless to a feed consumer's firewall, and unwrapping would be a second
  route into the store for the addresses the rule exists to keep out.
  Documentation ranges are deliberately kept — they are valid public space
  for this purpose and a live fleet reports them, and `2001:db8::/32` sits
  outside Teredo's `2001::/32` so keeping it needs no exception.

  **No width rule substitutes for this list.** Every address above is a
  single address, and `scope: Ip` already refuses a CIDR at ingest, so
  capping how broad a reported or exempted prefix may be leaves all of them
  untouched. The classification is the entire control, which is why an
  omission from `nonPublicPrefixes` is a data-protection bug: `::/96` was
  missing until 2026-09-10, and one reporter could store RFC1918 and the IMDS
  address by wrapping them.
  ...
  **A zoned address is refused before any of that, and the order is the
  ...
  `nonPublicPrefixes` or `IsUnspecified`, which is exactly every prefix
  listed above, used to sail through on a `%eth0` suffix
